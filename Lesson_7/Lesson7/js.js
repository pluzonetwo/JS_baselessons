...
const map = {
    ...
    render(snakePointsArray, foodPoint, walls) {
        ...
        walls.forEach((wall) => {
            const wallPoint = wall.getCoordinates();
...
const game = {
        ...
        reset() {
            ...
            this.wall.forEach((item) => {
                item.setCoordinates(this.getRandomFreeCoordinates());
            });
            this.renderScore();
            this.render();
        },
        ...
        getRandomFreeCoordinates() {
            const exclude = [this.food.getCoordinates(),
                ...this.wall.map((wallItem) => wallItem.getCoordinates()),
                ...this.snake.getBody()];
        ...
        render() {
            this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.wall);
        }
        ,
        ...
        tickHandler() {
            ...
            for (let i = 0; i < this.wall.length; i++) {
                if (this.wall[i].isOnPoint(this.snake.getNextStepHeadPoint())) {
                    this.finish();
                    break;
                }
            }
        ...
        renderScore() {
            ...
            score.innerText = `Ваш счет ${this.snake.getBody().length - 1}`;
        },
